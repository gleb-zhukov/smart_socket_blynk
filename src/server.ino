...
void mainForm() {
  ...
  Page += F(
            ...
            "<title>grib socket</title><meta http-equiv='Content-type' content='text/html; charset=utf-8'>"
            "<style type='text/css'>"
            "body {"
            "margin: 0;"
            "width: 100vw;"
            ...
            "<div class='brand-title'>grib socket</div>"
            "<div class='inputs'>"
            ...
            "<input type = 'text' name = 'name' required placeholder = 'Торшер'/>"

            "<label>Название комнаты</label>"
            "<input type = 'text' name = 'room' required placeholder = 'Спальня'/>"
...
void handleForm() {
  ...
  char name[41]; //название данного устройства
  char room[41]; //название данного устройства
  ...
  webServer.arg("name").toCharArray(name, sizeof(name) - 1);
  webServer.arg("room").toCharArray(room, sizeof(room) - 1);


  bool mode_flag_eeprom = 0; //включаем обычный режим (выключаем режим сервера)


  byte device_type[20]; //флаг включения/отключения пункта меню в клавиатуре розеток
  ...
  for (int i = 0; i < 41; i++) {
    room_name[0][i] = room[i];
  }

  for (int i = 0; i < 41; i++) {
    device_name[0][i] = name[i];
  }
  room_device[0][0] = 1;
  device_type[0] = 1;

  //записываем переменные в EEPROM
  EEPROM.put(3550, mode_flag_eeprom);
  EEPROM.commit();
